...
Usage:
    ...
    hermes uninstall           Uninstall Agente Hermes
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Agente Hermes'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_version(args):
    """Show version."""
    print(f"Agente Hermes v{__version__} ({__release_date__})")
...
def _update_via_zip(args):
    """Update Agente Hermes by downloading a ZIP archive.
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Agente Hermes — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def cmd_update(args):
    """Update Agente Hermes to the latest version.
    ...
    if is_managed():
        managed_error("update Agente Hermes")
        return
...
def main():
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Agente Hermes with an interactive wizard. "
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Agente Hermes components",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Agente Hermes setup",
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Agente Hermes. Use 'hermes debug share' to "
...
""",
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Agente Hermes configuration",
    )
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Agente Hermes to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Agente Hermes",
        description="Remove Agente Hermes from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Agente Hermes as an ACP (Agent Client Protocol) server",
        description="Start Agente Hermes in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Agente Hermes as an ACP server."""
        try:
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Agente Hermes web dashboard for managing config, API keys, and sessions",
    )
