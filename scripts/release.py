...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/aguiavisiontec/agente-hermes",
    ...
    lines.append(f"# Agente Hermes v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Agente Hermes. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Agente Hermes Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Agente Hermes Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Agente Hermes v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Agente Hermes v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Agente Hermes v{new_version} ({calver_date})' "
