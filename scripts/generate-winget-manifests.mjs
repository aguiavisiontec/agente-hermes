...
// the result under dist/winget/manifests/a/Aguiavisiontec/AgenteAguiavitech/<version>/.
//
// Run from CLI: VERSION=0.2.3 PUBLISH_OWNER=aguiavisiontec node scripts/generate-winget-manifests.mjs
...
export function generateWingetManifests({
  ...
  const installerUrl = `https://github.com/${publishOwner}/agente-hermes/releases/download/v${version}/${name}-${version}-setup.exe`;
  const releaseNotesUrl = `https://github.com/${publishOwner}/agente-hermes/releases/tag/v${version}`;
  ...
  const files = [
    ["Installer.template.yaml", "Aguiavisiontec.AgenteAguiavitech.installer.yaml"],
    [
      "Locale.en-US.template.yaml",
      "Aguiavisiontec.AgenteAguiavitech.locale.en-US.yaml",
    ],
    ["Version.template.yaml", "Aguiavisiontec.AgenteAguiavitech.yaml"],
  ];
...
if (isCli) {
  ...
  const result = generateWingetManifests({
    ...
    name: pkg.name,
    publishOwner: process.env.PUBLISH_OWNER || "aguiavisiontec",
  });
